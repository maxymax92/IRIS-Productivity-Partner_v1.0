...
export const getMediaCategory = (data: AttachmentData): AttachmentMediaCategory => {
  ...
  const mediaType = 'mediaType' in data && typeof data.mediaType === 'string' ? data.mediaType : ''
...
export const getAttachmentLabel = (data: AttachmentData): string => {
  ...
  const fn = 'filename' in data ? data.filename : undefined
  if (fn != null && fn !== '') return fn
  return category === 'image' ? 'Image' : 'Attachment'
}

const renderAttachmentImage = (url: string, filename: string | undefined, isGrid: boolean) =>
  isGrid ? (
    // eslint-disable-next-line @next/next/no-img-element -- blob/external URLs; next/image unsuitable for dynamic user content
    <img
      alt={filename ?? 'Image'}
...
export const Attachment = ({ data, onRemove, className, children, ...props }: AttachmentProps) => {
  ...
  const contextValue = useMemo<AttachmentContextValue>(
    () => ({ data, mediaCategory, ...(onRemove !== undefined && { onRemove }), variant }),
      ...
      <div
        className={cn(
          ...
          variant === 'inline' && [
            ...
            'border-border/40 rounded-md border px-1.5',
...
export const AttachmentPreview = ({
  ...
  const renderIcon = (iconComponent: typeof ImageIcon) => {
    const Icon = iconComponent
    return <Icon className={cn(iconSize, 'text-muted-foreground')} />
  }
  ...
  return (
    <div
      className={cn(
        ...
        variant === 'grid' && 'bg-surface size-full',
        variant === 'inline' && 'bg-background size-5 rounded',
        variant === 'list' && 'bg-surface size-12 rounded',
        className,
      )}
      {...props}
    >
