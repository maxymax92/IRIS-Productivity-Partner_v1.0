...
function getStatusBadge(state: ToolUIPart['state']) {
  const label = STATUS_LABELS[state] ?? state
  const variant = state === 'output-error' ? ('destructive' as const) : ('accent' as const)
  return (
...
export const SandboxTabsBar = ({ className, ...props }: SandboxTabsBarProps) => (
  <div
    className={cn('border-border/40 flex w-full items-center border-t border-b', className)}
    {...props}
  />
)
...
export const SandboxTabsTrigger = ({ className, ...props }: SandboxTabsTriggerProps) => (
  <TabsTrigger
    className={cn(
      'text-muted-foreground data-[state=active]:border-accent-strong data-[state=active]:text-foreground rounded-none border-0 border-b-2 border-transparent px-4 py-2 text-sm font-medium transition-colors data-[state=active]:bg-transparent data-[state=active]:shadow-none',
      className,
    )}
    {...props}
  />
