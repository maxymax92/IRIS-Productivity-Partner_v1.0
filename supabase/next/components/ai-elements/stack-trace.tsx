  ...
  ({
    ...
    return (
      <StackTraceContext.Provider value={contextValue}>
        <div
          className={cn(
            'not-prose bg-surface w-full overflow-hidden rounded-lg border font-mono text-sm',
            className,
          )}
          {...props}
        >
  ...
  return (
    <Collapsible onOpenChange={setIsOpen} open={isOpen}>
      <CollapsibleTrigger asChild {...props}>
        <div
          className={cn(
            'hover:bg-surface-raised/50 flex w-full cursor-pointer items-center gap-3 p-3 text-left transition-colors',
            className,
          )}
        >
          {children}
  ...
  ({ className, maxHeight = 400, children, ...props }: StackTraceContentProps) => {
    ...
    return (
      <Collapsible open={isOpen}>
        <CollapsibleContent
          className={cn(
            'bg-surface/30 overflow-auto border-t',
...
const FilePathButton = memo(({ frame, onFilePathClick }: FilePathButtonProps) => {
  ...
  return (
    <button
      className={cn(
        'hover:text-accent-strong underline decoration-dotted',
